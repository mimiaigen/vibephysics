...
def filter_black_gaussians(input_path, threshold=0.1):
    ...
    # Filter condition: Splat is "black" if R, G, and B are all below threshold
    # We want to KEEP splats that are NOT black
    ...
    print(f"Filtered points: {num_filtered} (Removed {num_original - num_filtered} black points)")
    
    # Create new vertex data with filtered points
    filtered_vertex_data = vertex.data[keep_mask].copy()
    
    if num_filtered == 0:
        print("Warning: All points were filtered out. No file will be saved.")
        return
    ...
    output_path = f"{base}_black_filtered{ext}"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Filter out black Gaussian splats from a PLY file.")
    parser.add_argument("input", help="Path to the input .ply file")
    parser.add_argument("-t", "--threshold", type=float, default=0.2, 
                        help="Threshold for blackness (0.0 to 1.0). Splats with R, G, B < threshold are removed. Default: 0.1")
    ...
    if not os.path.exists(args.input):
        ...
        filter_black_gaussians(args.input, args.threshold)
